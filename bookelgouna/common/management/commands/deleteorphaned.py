...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        for app in settings.ORPHANED_APPS_MEDIABASE_DIRS.keys():
            if 'root' in settings.ORPHANED_APPS_MEDIABASE_DIRS[app]:
                skip = settings.ORPHANED_APPS_MEDIABASE_DIRS[app].get('skip', ())
                exclude = settings.ORPHANED_APPS_MEDIABASE_DIRS[app].get('exclude', ())
                ...
                for model in ContentType.objects.filter(app_label=app):
                    ...
                    if len(fields) > 0:
                        ...
                        needed_files.extend([os.path.join(settings.MEDIA_ROOT, f)
                                             for f in filter(None, chain.from_iterable(files))])
                ...
                app_roots = settings.ORPHANED_APPS_MEDIABASE_DIRS[app]['root']
