...
from .forms import TransportCommentForm, TransportGalleryImageCropForm, TransportFeaturedImageCropForm, \
    TransportItemImageCropForm, TransportItemGalleryImageCropForm, TransportSearchForm, TransportReviewForm, \
    ItemSearchForm, CartItemForm
from .models import Transport, TransportReview, TransportComment, TransportItem, TransportImage, \
    TransportItemImage


class TransportDetailView(SetDatesAndGuestsIntoSessionMixin, ReservationPhoneToContextMixin, ActiveTabMixin,
                          DetailView):
    active_tab = 'transport'
    template_name = 'transport/transport_detail.html'
    ajax_template_name = 'transport/includes/ajax_transport_detail.html'
    ajax_template_name_time_based_only = 'transport/includes/ajax_transport_detail_time_based_only.html'
    queryset = Transport.objects.prefetch_related('images')
    comments_paginated_by = 10
    slug_field = 'slug__iexact'
    context_object_name = 'transport'
    business_type = BusinessOwnerInfo.TRANSPORT

    def get_object(self, queryset=None):
        if not hasattr(self, '_object'):
            self._object = super(TransportDetailView, self).get_object(queryset)
        return self._object
    ...
    # two types of items - item-based type is depended on header (sessions) dates only but
    ...
    def get_context_data(self, **kwargs):
        context = super(TransportDetailView, self).get_context_data(**kwargs)
        ...
        if has_items:
            ...
            context['one_ride_items'] = original.one_ride_items().filter(show_on_site=True).prefetch_related(
                'images', 'translations')
            ...
            if valid_time_based_dates:
                ...
                context['rent_items'] = original.rent_items().filter(show_on_site=True).prefetch_related(
                    'images', 'translations', 'discount_prices')

        comments = TransportComment.displayable.filter(entity_id=self.object.pk).select_related("creator")
        ...
        if user.is_authenticated() and user.is_end_user() and not user.is_staff:
            has_comments_to_this_transport = self.request.user.transport_comments.filter(entity_id=self.object.pk).exists()
            context['can_add_comment'] = not has_comments_to_this_transport
            transport_review = self.request.user.transport_reviews.filter(service_id=self.object.pk).first()
            context['show_rate_block'] = True
            context['can_review'] = not transport_review
            if transport_review:
                context['user_review'] = transport_review.rate
        ...
        if context['can_add_comment']:
            ...
            context['comment_form'] = TransportCommentForm(prefix='comment', initial=initial_comment_form)
        return context
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if staff_or_real_owner:
            if not request.is_ajax() and transport.status != Transport.APPROVED:
                messages.warning(request,
                                 pgettext_lazy('transport detail page not approved service warning message',
                                               'This transport is not approved and can be viewed by you only.'))
        else:
            if not (transport.is_origin() and transport.status == Transport.APPROVED and
                    (transport.has_prices() or transport.review_mode)):
                raise Http404()
        return super(TransportDetailView, self).dispatch(request, *args, **kwargs)


view_transport = TransportDetailView.as_view()


class TransportListView(SetDatesAndGuestsIntoSessionMixin, ActiveTabMixin, FormView):
    active_tab = 'transport'
    form_class = TransportSearchForm
    template_name = 'transport/find_transport.html'
    ajax_list_only_template_name = 'transport/includes/ajax_find_transport_list_only.html'
    ajax_list_with_filters_template_name = 'transport/includes/ajax_find_transport_list_with_filters.html'
    paginated_by = 10

    def get_initial(self):
        initial = super(TransportListView, self).get_initial()
        initial.update({
    ...
    def get_context_data(self, **kwargs):
        ...
        context['has_more_transports'] = self.has_more_transports
        return context

    @cached_property
    def type(self):
        return self.request.GET.getlist('type', [c[0] for c in TransportItem.TYPES])

    def get_queryset(self):
        # filtering here is done on per item basis only (selected number of days is not taken into account)
        q = Transport.originals.all()
        print q
        ...
        self.has_more_transports = q.count() > to_index
    ...
    def price_range(self):
        price_range = TransportItem.objects.filter(show_on_site=True, type__in=self.type).aggregate(min=Min('price'),
                                                                                                    max=Max('price'))
...
find_transport = TransportListView.as_view()
...
class AddItemsToCart(FormView):
    ...
    def get(self, request, *args, **kwargs):
        return HttpResponseRedirect(reverse('find_transport'))
    ...
    def form_valid(self, form):
        ...
        content_type = ContentType.objects.get_for_model(TransportItem)
        for k, v in self.request.POST.items():
            if k.startswith("i_"):
                cat_pk = k.split("i_", 1)[1]
                quantity = int(v)
                if quantity > 0:
                    trans_item = TransportItem.objects.get(pk=cat_pk)
                    object_id = trans_item.pk
                    if trans_item.is_one_ride_item:
...
class TransportGalleryImageCropView(BusinessOwnerOnlyMixin, BusinessProfileTabMixin, JsonMixin, UpdateView):
    ajax_template_name = "transport/includes/ajax_transport_gallery_image_crop.html"
    tab_name = "service"
    model = TransportImage
    form_class = TransportGalleryImageCropForm
    success_url = reverse_lazy('update_service')

    def form_valid(self, form):
        """
        It's required to explicitly change transport status so that this changes will be visible and could be
        approved by admin.
        """
        self.object = form.save()
        if self.object.service.status == Transport.UPDATING:
            self.object.service.status = Transport.ON_MODERATION
            self.object.service.save()
        return super(TransportGalleryImageCropView, self).form_valid(form)

transport_gallery_image_crop = TransportGalleryImageCropView.as_view()


class TransportFeaturedImageCropView(BusinessOwnerOnlyMixin, BusinessProfileTabMixin, JsonMixin, UpdateView):
    ajax_template_name = "transport/includes/ajax_transport_featured_image_crop.html"
    tab_name = "service"
    model = Transport
    form_class = TransportFeaturedImageCropForm
    success_url = reverse_lazy('update_service')
    ...
    def form_valid(self, form):
        ...
        if self.object.status == Transport.UPDATING:
            self.object.status = Transport.ON_MODERATION
        self.object.save()
        return super(TransportFeaturedImageCropView, self).form_valid(form)


transport_featured_image_crop = TransportFeaturedImageCropView.as_view()


class TransportItemGalleryCropView(BusinessOwnerOnlyMixin, BusinessProfileTabMixin, JsonMixin, UpdateView):
    ajax_template_name = "transport/includes/ajax_transport_item_gallery_image_crop.html"
    tab_name = "service"
    model = TransportItemImage
    form_class = TransportItemGalleryImageCropForm

    def get_success_url(self):
        return reverse('update_item', kwargs={"slug": self.object.service.slug})

transport_item_gallery_image_crop = TransportItemGalleryCropView.as_view()


class TransportItemImageCropView(BusinessOwnerOnlyMixin, BusinessProfileTabMixin, JsonMixin, UpdateView):
    ajax_template_name = "transport/includes/ajax_transport_item_image_crop.html"
    tab_name = "service"
    model = TransportItem
    form_class = TransportItemImageCropForm

    def get_success_url(self):
        return reverse('update_item', kwargs={"slug": self.object.slug})

transport_item_image_crop = TransportItemImageCropView.as_view()
        ...
        if new_val is not None and new_val >= 0:
            ...
            if item_slug:
                try:
                    ...
                    data['status'] = 'fail'
                    data['message'] = 'There is no transport item with this slug.'
                else:
...
class NewTransportCommentView(EndUserOnlyMixin, AjaxNewServiceCommentMixin, View):
    form_class = TransportCommentForm
    model = TransportComment

new_transport_comment = NewTransportCommentView.as_view()


class TransportCommentList(CommentList, View):
    ajax_template_name = "transport/comment_list.html"
    model = TransportComment
    paginated_by = 20


comment_list = TransportCommentList.as_view()


class NewTransportReviewView(EndUserOnlyMixin, AjaxNewReviewMixin, View):
    ajax_template_name = "transport/includes/ajax_review_block.html"
    service_model = Transport
    review_model = TransportReview
    form_class = TransportReviewForm

new_transport_review = NewTransportReviewView.as_view()
