...
from users.tests.factories import create_transport_business_owner

from ..models import Transport, TransportImage


@override_settings(MEDIA_ROOT=os.path.join(settings.BASE_DIR, 'testdata/media/'))
class TransportModerationTestCase(TestCase):

    def setUp(self):
        self.owner = create_transport_business_owner()
        o = Transport()
        o.owner = self.owner
        ...
        o.translate('en')
        o.title = 'Holms Cab'
        o.long_description = 'Nice fast cool Cab'
        o.slug = o.generate_slug(commit=True)

    def test_duplicate_creation(self):
        o = Transport.objects.first()
        original_pk_before_dup = o.pk
        d = o.create_object_duplicate_from_given()
        d = Transport.objects.get(pk=d.pk)  # refresh it from db
        ...
        # 3: check related objects of duplicate and original transport
        # 3.1: translations
        d_trans = Transport.objects.language('en').get(pk=d.pk)
        self.assertEqual(d_trans.language_code, 'en')
        self.assertEqual(d_trans.title, 'Holms Cab')
        self.assertEqual(d_trans.long_description, 'Nice fast cool Cab')
        o_trans = Transport.objects.language('en').get(pk=o.pk)
        self.assertEqual(o_trans.language_code, 'en')
        self.assertEqual(o_trans.title, 'Holms Cab')
        self.assertEqual(o_trans.long_description, 'Nice fast cool Cab')
    ...
    def test_moderation_and_approving(self):
        ...
        d = o.create_object_duplicate_from_given()
        old_dup_pk = d.pk
        Transport.rewrite_origin_object_with_approved_duplicate(d.origin, d)
        self.assertRaises(Transport.DoesNotExist, Transport.objects.get, pk=old_dup_pk)  # old duplicate does not exist
        # there is no related objects of duplicate anymore
        dup_images = TransportImage.objects.filter(service_id=old_dup_pk)
