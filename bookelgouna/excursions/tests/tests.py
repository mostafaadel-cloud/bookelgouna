...
from users.tests.factories import create_excursion_business_owner

from ..models import Excursion, ExcursionImage


@override_settings(MEDIA_ROOT=os.path.join(settings.BASE_DIR, 'testdata/media/'))
class ExcursionModerationTestCase(TestCase):

    def setUp(self):
        self.owner = create_excursion_business_owner()
        o = Excursion()
        o.owner = self.owner
        ...
        o.translate('en')
        o.title = 'Excursion #1'
        o.long_description = 'Excursion company #1'
        o.slug = o.generate_slug(commit=True)

    def test_duplicate_creation(self):
        o = Excursion.objects.first()
        original_pk_before_dup = o.pk
        d = o.create_object_duplicate_from_given()
        d = Excursion.objects.get(pk=d.pk)  # refresh it from db
        ...
        # 3: check related objects of duplicate and original excursion
        # 3.1: translations
        d_trans = Excursion.objects.language('en').get(pk=d.pk)
        self.assertEqual(d_trans.language_code, 'en')
        self.assertEqual(d_trans.title, 'Excursion #1')
        self.assertEqual(d_trans.long_description, 'Excursion company #1')
        o_trans = Excursion.objects.language('en').get(pk=o.pk)
        self.assertEqual(o_trans.language_code, 'en')
        self.assertEqual(o_trans.title, 'Excursion #1')
        self.assertEqual(o_trans.long_description, 'Excursion company #1')
    ...
    def test_moderation_and_approving(self):
        ...
        d = o.create_object_duplicate_from_given()
        old_dup_pk = d.pk
        Excursion.rewrite_origin_object_with_approved_duplicate(d.origin, d)
        self.assertRaises(Excursion.DoesNotExist, Excursion.objects.get, pk=old_dup_pk)  # old duplicate does not exist
        # there is no related objects of duplicate anymore
        dup_images = ExcursionImage.objects.filter(service_id=old_dup_pk)
