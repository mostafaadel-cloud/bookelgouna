...
from .forms import ExcursionGalleryImageCropForm, ExcursionFeaturedImageCropForm, ExcursionItemGalleryImageCropForm, \
    ExcursionItemImageCropForm, ExcursionCommentForm, ExcursionReviewForm, ExcursionSearchForm, CartItemForm, \
    ItemSearchForm
from .models import ExcursionImage, Excursion, ExcursionItemImage, ExcursionItem, ExcursionComment, ExcursionReview


class ExcursionDetailView(SetDatesAndGuestsIntoSessionMixin, ReservationPhoneToContextMixin, ActiveTabMixin,
                          DetailView):
    active_tab = 'excursions'
    template_name = 'excursions/excursion_detail.html'
    ajax_template_name = 'excursions/includes/ajax_excur_detail.html'
    ajax_template_name_time_based_only = 'excursions/includes/ajax_excur_detail_time_based_only.html'
    queryset = Excursion.objects.prefetch_related('images')
    comments_paginated_by = 10
    slug_field = 'slug__iexact'
    context_object_name = 'excursion'
    business_type = BusinessOwnerInfo.EXCURSION

    def get_object(self, queryset=None):
        if not hasattr(self, '_object'):
            self._object = super(ExcursionDetailView, self).get_object(queryset)
        return self._object
    ...
    def get_context_data(self, **kwargs):
        context = super(ExcursionDetailView, self).get_context_data(**kwargs)
        ...
        if has_items:
            ...
            if valid_time_based_dates:
                ...
                context['tour_items'] = original.tour_items().filter(show_on_site=True).prefetch_related(
                    'images', 'translations', 'discount_prices')

        comments = ExcursionComment.displayable.filter(entity_id=self.object.pk).select_related("creator")
        ...
        if user.is_authenticated() and user.is_end_user() and not user.is_staff:
            has_comments_to_this_excursion = self.request.user.excursion_comments.filter(entity_id=self.object.pk).exists()
            context['can_add_comment'] = not has_comments_to_this_excursion
            excursion_review = self.request.user.excursion_reviews.filter(service_id=self.object.pk).first()
            context['show_rate_block'] = True
            context['can_review'] = not excursion_review
            if excursion_review:
                context['user_review'] = excursion_review.rate
        ...
        if context['can_add_comment']:
            ...
            context['comment_form'] = ExcursionCommentForm(prefix='comment', initial=initial_comment_form)
        return context
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        staff_or_real_owner = user.is_authenticated() and (user.is_staff or (user.is_business_owner() and
                                                                             excursion.owner == user))
        if staff_or_real_owner:
            if not request.is_ajax() and excursion.status != Excursion.APPROVED:
                messages.warning(request,
                                 pgettext_lazy('excursion detail page not approved service warning message',
                                               'This excursion is not approved and can be viewed by you only.'))
        else:
            if not (excursion.is_origin() and excursion.status == Excursion.APPROVED and
                    (excursion.has_prices() or excursion.review_mode)):
                raise Http404()
        return super(ExcursionDetailView, self).dispatch(request, *args, **kwargs)


view_excursion = ExcursionDetailView.as_view()


class ExcursionListView(SetDatesAndGuestsIntoSessionMixin, ActiveTabMixin, FormView):
    active_tab = 'excursions'
    form_class = ExcursionSearchForm
    template_name = 'excursions/find_excursion.html'
    ajax_list_only_template_name = 'excursions/includes/ajax_find_excur_list_only.html'
    ajax_list_with_filters_template_name = 'excursions/includes/ajax_find_excur_list_with_filters.html'
    paginated_by = 10

    def get_initial(self):
        initial = super(ExcursionListView, self).get_initial()
        initial.update({
    ...
    def get_context_data(self, **kwargs):
        context = super(ExcursionListView, self).get_context_data(**kwargs)
        ...
        context['has_more_excursions'] = self.has_more_excursions
        return context

    @cached_property
    def type(self):
        return self.request.GET.getlist('type', [c[0] for c in ExcursionItem.TYPES])

    def get_queryset(self):
        # filtering here is done on per item basis only (selected number of days is not taken into account)
        q = Excursion.originals.all()
        ...
        self.has_more_excursions = q.count() > to_index
    ...
    def price_range(self):
        price_range = ExcursionItem.objects.filter(show_on_site=True, type__in=self.type).aggregate(min=Min('price'),
                                                                                                    max=Max('price'))
...
find_excursion = ExcursionListView.as_view()
...
class AddItemsToCart(FormView):
    ...
    def get(self, request, *args, **kwargs):
        return HttpResponseRedirect(reverse('find_excursion'))
    ...
    def form_valid(self, form):
        ...
        content_type = ContentType.objects.get_for_model(ExcursionItem)
        for k, v in self.request.POST.items():
            if k.startswith("i_"):
                cat_pk = k.split("i_", 1)[1]
                quantity = int(v)
                if quantity > 0:
                    trans_item = ExcursionItem.objects.get(pk=cat_pk)
...
class ExcursionGalleryImageCropView(BusinessOwnerOnlyMixin, BusinessProfileTabMixin, JsonMixin, UpdateView):
    ajax_template_name = "excursions/includes/ajax_excur_gallery_image_crop.html"
    tab_name = "service"
    model = ExcursionImage
    form_class = ExcursionGalleryImageCropForm
    success_url = reverse_lazy('update_service')

    def form_valid(self, form):
        """
        It's required to explicitly change excursion status so that this changes will be visible and could be
        approved by admin.
        """
        self.object = form.save()
        if self.object.service.status == Excursion.UPDATING:
            self.object.service.status = Excursion.ON_MODERATION
            self.object.service.save()
        return super(ExcursionGalleryImageCropView, self).form_valid(form)

excursion_gallery_image_crop = ExcursionGalleryImageCropView.as_view()


class ExcursionFeaturedImageCropView(BusinessOwnerOnlyMixin, BusinessProfileTabMixin, JsonMixin, UpdateView):
    ajax_template_name = "excursions/includes/ajax_excur_featured_image_crop.html"
    tab_name = "service"
    model = Excursion
    form_class = ExcursionFeaturedImageCropForm
    success_url = reverse_lazy('update_service')
    ...
    def form_valid(self, form):
        ...
        if self.object.status == Excursion.UPDATING:
            self.object.status = Excursion.ON_MODERATION
        self.object.save()
        return super(ExcursionFeaturedImageCropView, self).form_valid(form)


excursion_featured_image_crop = ExcursionFeaturedImageCropView.as_view()


class ExcursionItemGalleryCropView(BusinessOwnerOnlyMixin, BusinessProfileTabMixin, JsonMixin, UpdateView):
    ajax_template_name = "excursions/includes/ajax_excur_item_gallery_image_crop.html"
    tab_name = "service"
    model = ExcursionItemImage
    form_class = ExcursionItemGalleryImageCropForm

    def get_success_url(self):
        return reverse('update_item', kwargs={"slug": self.object.service.slug})

excursion_item_gallery_image_crop = ExcursionItemGalleryCropView.as_view()


class ExcursionItemImageCropView(BusinessOwnerOnlyMixin, BusinessProfileTabMixin, JsonMixin, UpdateView):
    ajax_template_name = "excursions/includes/ajax_excur_item_image_crop.html"
    tab_name = "service"
    model = ExcursionItem
    form_class = ExcursionItemImageCropForm

    def get_success_url(self):
        return reverse('update_item', kwargs={"slug": self.object.slug})

excursion_item_image_crop = ExcursionItemImageCropView.as_view()
...
class RemoveItemView(BusinessOwnerOnlyMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if item_slug:
            ...
            except ExcursionItem.DoesNotExist:
                data['status'] = 'fail'
        ...
        if new_val is not None and new_val >= 0:
            ...
            if item_slug:
                try:
                    ...
                    data['status'] = 'fail'
                    data['message'] = 'There is no excursion item with this slug.'
                else:
...
class NewExcursionCommentView(EndUserOnlyMixin, AjaxNewServiceCommentMixin, View):
    form_class = ExcursionCommentForm
    model = ExcursionComment

new_excursion_comment = NewExcursionCommentView.as_view()


class ExcursionCommentList(CommentList, View):
    ajax_template_name = "excursions/comment_list.html"
    model = ExcursionComment
    paginated_by = 20


comment_list = ExcursionCommentList.as_view()


class NewExcursionReviewView(EndUserOnlyMixin, AjaxNewReviewMixin, View):
    ajax_template_name = "excursions/includes/ajax_review_block.html"
    service_model = Excursion
    review_model = ExcursionReview
    form_class = ExcursionReviewForm

new_excursion_review = NewExcursionReviewView.as_view()
