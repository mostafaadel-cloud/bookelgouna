...
from users.tests.factories import create_things_to_do_business_owner

from ..models import Entertainment, EntertainmentImage


@override_settings(MEDIA_ROOT=os.path.join(settings.BASE_DIR, 'testdata/media/'))
class EntertainmentModerationTestCase(TestCase):

    def setUp(self):
        self.owner = create_things_to_do_business_owner()
        o = Entertainment()
        o.owner = self.owner
        ...
        o.translate('en')
        o.title = 'Entertainment #1'
        o.long_description = 'Entertainment company #1'
        o.slug = o.generate_slug(commit=True)

    def test_duplicate_creation(self):
        o = Entertainment.objects.first()
        original_pk_before_dup = o.pk
        d = o.create_object_duplicate_from_given()
        d = Entertainment.objects.get(pk=d.pk)  # refresh it from db
        ...
        # 3: check related objects of duplicate and original entertainment
        # 3.1: translations
        d_trans = Entertainment.objects.language('en').get(pk=d.pk)
        self.assertEqual(d_trans.language_code, 'en')
        self.assertEqual(d_trans.title, 'Entertainment #1')
        self.assertEqual(d_trans.long_description, 'Entertainment company #1')
        o_trans = Entertainment.objects.language('en').get(pk=o.pk)
        self.assertEqual(o_trans.language_code, 'en')
        self.assertEqual(o_trans.title, 'Entertainment #1')
        self.assertEqual(o_trans.long_description, 'Entertainment company #1')
    ...
    def test_moderation_and_approving(self):
        ...
        d = o.create_object_duplicate_from_given()
        old_dup_pk = d.pk
        Entertainment.rewrite_origin_object_with_approved_duplicate(d.origin, d)
        self.assertRaises(Entertainment.DoesNotExist, Entertainment.objects.get, pk=old_dup_pk)  # old duplicate does not exist
        # there is no related objects of duplicate anymore
        dup_images = EntertainmentImage.objects.filter(service_id=old_dup_pk)
